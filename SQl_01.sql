...
-- 1. Count the number of Salespeople whose name begins with 'a'/'A'
SELECT COUNT(*)
FROM SalesPeople
WHERE UPPER(Sname) LIKE 'A%';

-- 2. Display all the Salespeople whose all orders are worth more than Rs. 2000
...
SELECT S.Snum, S.Sname, COUNT(O.Onum) AS OrderCount, O.Odate
FROM SalesPeople S
